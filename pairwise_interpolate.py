...
def lagrange_linear(xdata, ydata, xnew, precision=6):
    ...
    if not len(xdata) == len(ydata):
        print(("x-input has length %d, y-input has length !" %
               (len(xdata), len(ydata))))
        return None
    else:
        ...
        for j, ind in enumerate(pos_xnew):
            try:
                ...
                #print pos_xnew
                sys.exit(
                    'Either the input x data is not sorted in ascending order or you are trying to extrapolate rather than interpolate')
            if xd_actual == xnew_actual:
                ...
                try:
                    ...
                    print(("Trying to access element number %d out of %d elements in the data" % (
                        ind, len(ydata))))
                    #print pos_xnew
                ynew[j] = y_lower * ((x_upper - xnew[j]) / (x_upper - x_lower)) + \
                    y_upper * ((xnew[j] - x_lower) / (x_upper - x_lower))
